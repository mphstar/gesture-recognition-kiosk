  ...
  const handleSelectAll = () => {
    if(data){
      if (selectAll) {
        ...
        setSelectedRows(data.products.icecream.data.map((item) => item.id));
      }
          ...
          <div className="h-fit flex-grow flex flex-col py-4 px-6 md:px-12 bg-gray-100">
              ...
              <table className="border-separate border-spacing-y-3">
                ...
                <tbody className="bg-gray-100 rounded-xl">
                  {!error ? (
                    isLoading ? (
                    ...
                    ) : data.products.icecream.data.length == 0 ? (
                      <tr>
                        <td
                      ...
                      data.products.icecream.data.map((item, index) => {
                        return (
                          <motion.tr
            ...
            {isLoading || error ? undefined : (
              <Pagination
                page={page}
                SetPage={SetPage}
                total={!data ? 0 : data.products.icecream.total_data}
                showItem={!data ? 0 : data.products.icecream.data.length}
                limit={limit}
              />
            )}
          </div>
